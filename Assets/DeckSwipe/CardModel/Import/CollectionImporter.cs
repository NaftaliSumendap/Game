		...
		public async Task<ImportedCards> Import() {
			...
			foreach (ProtoImage image in collection.images) {
				...
				if (string.IsNullOrEmpty(image.localPath)) {
					Debug.LogWarning("[CollectionImporter] Image (id: " + image.id + ") has a null or empty localPath");
					continue;
				}

				// Muat sprite dari Resources
				Debug.Log("[CollectionImporter] Fetching image from " + image.localPath + " ...");
				Sprite sprite = Resources.Load<Sprite>(image.localPath.Replace("Assets/Resources/", "").Replace(".jpeg", "").Replace(".png", ""));
				
				if (sprite == null) {
					Debug.LogWarning("[CollectionImporter] Could not load sprite from localPath: " + image.localPath);
					continue;
				}

				// Tambahkan sprite ke dictionary
